 ...
 * @since 2.2
 */
function affwp_customers_screen_options() {

	$screen = affwp_get_current_screen();

	if ( $screen !== 'affiliate-wp-customers' ) {
		return;
	}
	...
	add_screen_option(
		'per_page',
		array(
			'label'   => __( 'Number of customers per page:', 'affiliate-wp' ),
			'option'  => 'affwp_edit_customers_per_page',
			'default' => 30,
		)
	);
	 ...
	 */
	do_action( 'affwp_customers_screen_options', $screen );
 ...
 * @return mixed
 */
function affwp_customers_set_screen_option( $status, $option, $value ) {

	if ( 'affwp_edit_customers_per_page' === $option ) {
		return $value;
	}
...
add_filter( 'set-screen-option', 'affwp_customers_set_screen_option', 10, 3 );
